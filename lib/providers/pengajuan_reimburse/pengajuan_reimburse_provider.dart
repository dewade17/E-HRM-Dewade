...
class PengajuanReimburseProvider extends ChangeNotifier {
  PengajuanReimburseProvider();
  ...
  String get _listEndpoint => Endpoints.reimburse;
  String _detailEndpoint(String id) => Endpoints.reimburseDetail(id);
  ...
  Future<dto.Data?> createReimburse({
    String? idDepartement,
    ...
    try {
      ...
      saveMessage =
          response['message']?.toString() ?? 'Berhasil membuat reimburse.';
      saving = false;
      notifyListeners();
      return created;
    } catch (e) {
  ...
  Future<dto.Data?> updateReimburse(
    String id, {
    DateTime? tanggal,
    String? keterangan,
    ...
    try {
      ...
      if (updated != null) {
        _upsertItem(updated);
        if (detail?.idReimburse == updated.idReimburse) detail = updated;
      }

      saveMessage =
          response['message']?.toString() ?? 'Berhasil memperbarui reimburse.';
      saving = false;
      notifyListeners();
      return updated;
    } catch (e) {
  ...
  Future<bool> deleteReimburse(String id) async {
    final trimmedId = id.trim();
    ...
    try {
      ...
      items.removeWhere((it) => it.idReimburse == trimmedId);
      if (detail?.idReimburse == trimmedId) detail = null;
  ...
  dto.Data? _parseSingle(dynamic raw) {
    ...
    if (raw is Map<String, dynamic>) {
      if (raw.containsKey('id_reimburse')) {
  ...
  void _upsertItem(dto.Data item) {
    final index = items.indexWhere((it) => it.idReimburse == item.idReimburse);
    if (index >= 0) {
      items[index] = item;
    } else {
      items.insert(0, item);
          ...
          .map((e) {
            if (e is Map) {
              return <String, dynamic>{
                'id_reimburse_item': safeStr(e['id_reimburse_item']),
                'nama_item_reimburse': safeStr(e['nama_item_reimburse']),
      ...
      normalized['approvals'] = rawApprovals
          ...
          .map((e) {
            if (e is Map) {
              ...
              return <String, dynamic>{
                'id_approval_reimburse': safeStr(e['id_approval_reimburse']),
                'id_reimburse': safeStr(e['id_reimburse']),
                ...
                'note': safeStr(e['note']),
                'bukti_approval_reimburse_url': safeStr(
                  e['bukti_approval_reimburse_url'],
                ),
                'approver': approverJson,
              };
            }
            return <String, dynamic>{
              'id_approval_reimburse': '',
              'id_reimburse': safeStr(normalized['id_reimburse']),
              'level': 0,
              'approver_user_id': '',
              'approver_role': '',
              'decision': 'pending',
