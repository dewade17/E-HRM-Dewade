...
import 'package:e_hrm/screens/users/finance/payment/detail_payment/detail_payment_screen.dart';
import 'package:e_hrm/screens/users/finance/payment/riwayat_payment/widget/calendar_riwayat_payment.dart';
...
class ContentRiwayatPayment extends StatefulWidget {
  const ContentRiwayatPayment({super.key});

  @override
  State<ContentRiwayatPayment> createState() => _ContentRiwayatPaymentState();
}

class _ContentRiwayatPaymentState extends State<ContentRiwayatPayment> {
  ...
  Future<void> _deleteItem(dto.Data item) async {
    ...
    final ok = await provider.deletePayment(item.idPayment);

    if (!mounted) return;

    if (!ok) {
      final msg = provider.deleteError ?? 'Gagal menghapus payment.';
  ...
  Widget _buildCard(dto.Data data) {
    ...
    final nominal = _formatRupiah(data.nominalPembayaran);
                            ...
                            : () {
                                showDialog(
                                  ...
                                  builder: (_) => ConfirmationDialog(
                                    title: "Hapus Payment?",
                                    subtitle:
                                        "Pengajuan payment ini akan dihapus.",
