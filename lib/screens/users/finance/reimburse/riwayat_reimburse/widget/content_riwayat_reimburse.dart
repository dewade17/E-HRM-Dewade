...
import 'package:e_hrm/screens/users/finance/reimburse/detail_reimburse/detail_reimburse_screen.dart';
import 'package:e_hrm/screens/users/finance/reimburse/riwayat_reimburse/widget/calendar_riwayat_reimburse.dart';
...
class ContentRiwayatReimburse extends StatefulWidget {
  const ContentRiwayatReimburse({super.key});

  @override
  State<ContentRiwayatReimburse> createState() =>
      _ContentRiwayatReimburseState();
}

class _ContentRiwayatReimburseState extends State<ContentRiwayatReimburse> {
  ...
  Future<void> _applyDateFilter(DateTime? day) async {
    final provider = context.read<PengajuanReimburseProvider>();
  ...
  Future<void> _deleteItem(dto.Data item) async {
    ...
    final ok = await provider.deleteReimburse(item.idReimburse);

    if (!mounted) return;

    if (!ok) {
      final msg = provider.deleteError ?? 'Gagal menghapus reimburse.';
    ...
    ScaffoldMessenger.of(context)
      ...
      ..showSnackBar(
        const SnackBar(
          content: Text('Reimburse berhasil dihapus.'),
                            ...
                            : () {
                                showDialog(
                                  ...
                                  builder: (_) => ConfirmationDialog(
                                    ...
                                    subtitle:
                                        "Pengajuan reimburse ini akan dihapus.",
                ...
                GestureDetector(
                  onTap: () async {
                    await Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => DetailReimburseScreen(
                          idReimburse: data.idReimburse,
                          initialData: data,
                        ),
                      ),
                    );
  ...
  Widget build(BuildContext context) {
    return Consumer<PengajuanReimburseProvider>(
      builder: (context, provider, _) {
        ...
        return Column(
          children: [
            CalendarRiwayatReimburse(
              selectedDay: _selectedDay,
              markerDates: markerDates,
              onDaySelected: (day) => _applyDateFilter(day),
            ),
            ...
            if (provider.loading && provider.items.isEmpty)
              ...
              Padding(
                padding: const EdgeInsets.symmetric(vertical: 16),
                child: Text(
                  "Tidak ada reimburse sesuai filter.",
                  style: GoogleFonts.poppins(
                    fontSize: 13,
                    fontWeight: FontWeight.w500,
                    color: Colors.black54,
